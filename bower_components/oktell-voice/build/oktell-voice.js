		...
		var RTCMediaHandler = (function (JsSIP) {
			...
			RTCMediaHandler.prototype = {
				...
				init: function (constraints) {
					...
					oktellVoice.peer = this.peerConnection;
...
//							this.localMedia.stop();
				...
				getUserMedia: function (ua,onSuccess, onFailure, constraints) {
					...
					var onSuccessGetUserMedia = function (stream) {
						...
						window.currentLocalStream1 = stream
...
oktellVoice = (function() {
      ...
      this.UA.on('newRTCSession', function(e) {
        ...
        onSessionStart = function(e) {
          ...
          if (_this.currentSession.getLocalStreams().length > 0) {
            console.log('currentSession local stream > 0', _this.currentSession.getLocalStreams()[0]);
			  okVoice.currentLocalStream = _this.currentSession.getLocalStreams()[0];
			  _this.elLocal.src = window.URL.createObjectURL(_this.currentSession.getLocalStreams()[0]);
          } else {
			  console.log('currentSession local stream == 0');
          }
          if (_this.currentSession.getRemoteStreams().length > 0) {
			  console.log('currentSession remote stream > 0', _this.currentSession.getRemoteStreams()[0]);
			  okVoice.currentRemoteStream = _this.currentSession.getRemoteStreams()[0];
            _this.elRemote.src = window.URL.createObjectURL(_this.currentSession.getRemoteStreams()[0]);
			  okVoice.remoteEl = _this.elRemote;
            return _this.elRemote.play();
          } else {
            return console.log('currentSession remote stream == 0');
          }
        };
  ...
  okVoice.isSupported = function() {
	...
	okVoice.getUserMediaStream = function() {
		return okVoice._localMediaStream || userMedia;
	};
  okVoice.createUserMedia = function(onSuccess, onDeny, useVideo) {
    var getUserMedia, hasDecision, triggerDeny;
    if (okVoice.getUserMediaStream()) {
      return typeof onSuccess === "function" ? onSuccess(okVoice.getUserMediaStream()) : void 0;
    }
